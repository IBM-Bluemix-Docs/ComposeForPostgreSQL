...
This tutorial uses the [compose-postgresql-helloworld-nodejs](https://github.com/IBM-Cloud/compose-postgresql-helloworld-nodejs) sample app to demonstrate how to use Node.js to connect to a {{site.data.keyword.composeForPostgreSQL_full}} service using the provided credentials. The application creates, reads from, and writes to a database using data supplied through the app's web interface.
{: shortdesc}
